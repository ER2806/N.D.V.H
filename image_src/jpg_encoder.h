...
private:
    ...
    size_t variable_elem_index = 10;                 // Element to write bit index
    std::vector<std::bitset<8>> bitset_y_components; // Blue color bits
    // structs from jpeglib
    struct jpeg_decompress_struct cinfo;
    //struct jpeg_error_mgr jerr;
    jvirt_barray_ptr *coeffs_array;
    jpeg_component_info* compptr_one;              // Matrix of information about component Y (CrCb)

    FILE * infile = NULL;
    FILE * outfile = NULL;
    JBLOCKARRAY DCT_coefficients;                  // DCT coefs
